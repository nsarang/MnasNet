...
def MnasBlock(input_tensor, block_args, global_params, name):
    ...
    if block_args.expand_ratio != 1:
      ...
      x = tf.keras.layers.Conv2D(
        ...
        name=name+'_expand_conv')(input_tensor)
    ...
    x = tf.keras.layers.BatchNormalization(
        ...
        epsilon=batch_norm_epsilon,
        fused=True,
        name=name+'_depthwise_conv_BN')(x)
    x = tf.keras.layers.ReLU()(x)
    ...
    if has_se:
      ...
      se_tensor = tf.reduce_mean(x, _spatial_dims, keepdims=True)
      se_tensor = tf.keras.layers.Conv2D(
          ...
          use_bias=True,
          data_format=data_format,
          name =name+'_se_reduce') (se_tensor)
      ...
      se_tensor = tf.keras.layers.Conv2D(
          ...
          use_bias=True,
          data_format=data_format,
          name=name+'_se_expand')(se_tensor)
