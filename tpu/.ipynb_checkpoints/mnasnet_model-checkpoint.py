...
def conv_kernel_initializer(shape, dtype=None, partition_info=None):
  ...
  return np.random.normal(
      loc=0.0, scale=np.sqrt(2.0 / fan_out), size=shape)
...
class MnasBlock(object):
  ...
  def _call_se(self, input_tensor):
    ...
    tf.logging.info('Built Squeeze and Excitation with tensor shape: %s' %
                    (se_tensor.shape))
    return tf.nn.sigmoid(se_tensor) * input_tensor
...
class MnasNetModel(tf.keras.Model):
  ...
  def call(self, inputs, training=True, features_only=None):
    ...
    # Calls Stem layers
    print(type(inputs))
    print(type(self._bn0(self._conv_stem(inputs), training=training)))
    print(self._bn0(self._conv_stem(inputs), training=training))
    outputs = tf.nn.relu(
